...
const Bodygraph = {
  ...
  // Строим граф связей между определёнными центрами
  buildGraph: function(activeGates) {
    const gateToCenter = {};
    for(const [c, data] of Object.entries(this.CENTERS))
      for(const g of data.gates) gateToCenter[g] = c;

    const graph = {};
    for(const c of Object.keys(this.CENTERS)) graph[c] = new Set();

    this.CHANNELS.forEach(([g1,g2]) => {
      if(activeGates.has(g1) && activeGates.has(g2)) {
        const c1 = gateToCenter[g1], c2 = gateToCenter[g2];
        if(c1 && c2 && c1 !== c2) { graph[c1].add(c2); graph[c2].add(c1); }
      }
    });
    return graph;
  },

  // Проверка связности двух центров через BFS
  isConnected: function(graph, from, to) {
  ...
  getType: function(ac, activeGates) {
    const anyDefined = Object.values(ac).some(v => v);
    if (!anyDefined) return { type:'Рефлектор', strategy:'Ждать лунный цикл (28 дней)' };

    const graph = this.buildGraph(activeGates);

    // Манифестор = мотор (не сакральный) соединён с Горлом через цепочку
    if(ac.throat) {
      for(const motor of ['solar','will','root']) {
        if(ac[motor] && this.isConnected(graph, motor, 'throat')) {
          if(!ac.sacral) return { type:'Манифестор', strategy:'Информировать перед действием' };
  ...
  calculate: function(birthDate) {
    ...
    const typeData = this.getType(ac, ag);
    return {
      type:             typeData.type,
